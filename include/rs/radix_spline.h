...
class RadixSpline {
 public:
  ...
  size_t GetSize() const {
    std::cout << "#segments: " << spline_points_.size() << std::endl;
    std::cout << "radix table size: " << (radix_table_.size() * sizeof(uint32_t)) << std::endl;
 ...
 private:
  ...
  size_t GetSplineSegment(const KeyType key) const {
    ...
    std::cout << "IO: " << (double)(end - begin)/32.0 << std::endl;
    if (end - begin < 32) {
